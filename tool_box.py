...
class Plot():
    ...
    def plot_random_output(testloader_dataset, dataset, net):
        ...
        for idx,(i,j,k) in enumerate(zip(images, labels, preds)):
            ...
            ax = fig.add_subplot(4,4,idx)
            ax.imshow(i.squeeze().numpy())          
            title = j + ' predicted as ' + dataset.classes[k]
            ax.set_title(title)
    ...
    def class_accuracies(self, net, dataset, val_loader):
        ...
        return pd.DataFrame(np.column_stack([dataset.classes,accuracy_per_class]), columns=['Class', 'Accuracy'])
